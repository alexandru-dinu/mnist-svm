...
def grid_search(parameters, x_train, y_train):
    ...
    grid_clsf = GridSearchCV(
        estimator=svm_clsf, param_grid=parameters, n_jobs=-1, verbose=2)
...
def evaluate(classifier, x_test, y_test) -> None:
    ...
    show_some_digits(x_test, predicted, sample_size=24,
                     normalized=True, title_fmt="Predicted {}")
    plt.show()
...
def main() -> None:
    ...
    best_svm, _ = grid_search(params, x_train[idx_train].reshape(
        n_train, -1), y_train[idx_train])
